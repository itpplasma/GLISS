program enzyme_terpsichore_reduced_mass_family_gradient
    use, intrinsic :: iso_c_binding, only: c_double, c_funloc, c_funptr
    use, intrinsic :: iso_fortran_env, only: error_unit
    use enzyme_terpsichore_reduced_mass_family_fixture, only: &
        terpsichore_reduced_family_mass_energy
    implicit none
    ...
    gradient = enzyme_autodiff( &
        c_funloc(terpsichore_reduced_family_mass_energy), point)
    centered = (terpsichore_reduced_family_mass_energy(point + step) &
        - terpsichore_reduced_family_mass_energy(point - step)) &
        / (2.0_c_double * step)
    if (abs(gradient - centered) > 1.0e-8_c_double &
        * max(1.0_c_double, abs(centered))) then
        write (error_unit, "(a,2es24.16)") &
            "FAIL TERPSICHORE reduced family mass gradient ", &
            gradient, centered
        error stop 1
    end if
    write (*, "(a)") "PASS"
end program enzyme_terpsichore_reduced_mass_family_gradient
