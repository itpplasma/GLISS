program enzyme_physical_mass_gradient
    ...
    gradient = enzyme_autodiff(c_funloc(benchmark_physical_mass_energy), &
        0.1_c_double)
    ...
    if (abs(gradient - centered) > 1.0e-8_c_double &
        * max(1.0_c_double, abs(centered))) then
        write (error_unit, "(a,2es24.16)") &
            "FAIL physical mass gradient ", gradient, centered
        error stop 1
    end if
    write (*, "(a)") "PASS"
end program enzyme_physical_mass_gradient
