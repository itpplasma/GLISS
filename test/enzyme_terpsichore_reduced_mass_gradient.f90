program enzyme_terpsichore_reduced_mass_gradient
    ...
    gradient = enzyme_autodiff( &
        c_funloc(benchmark_terpsichore_reduced_mass_energy), 0.1_c_double)
    centered = (benchmark_terpsichore_reduced_mass_energy(0.1_c_double + step) &
        - benchmark_terpsichore_reduced_mass_energy(0.1_c_double - step)) &
        / (2.0_c_double * step)
    if (abs(gradient - centered) > 1.0e-8_c_double &
        * max(1.0_c_double, abs(centered))) then
        write (error_unit, "(a,2es24.16)") &
            "FAIL TERPSICHORE reduced mass gradient ", gradient, centered
        error stop 1
    end if
    write (*, "(a)") "PASS"
end program enzyme_terpsichore_reduced_mass_gradient
