program test_physical_mass_kernel
    ...
    real(dp) :: xi_contravariant(3), flux_t_slope, flux_p_slope
    ...
    current_j = dot_product(metric(2, 2:3), b_contravariant)
    current_i = dot_product(metric(3, 2:3), b_contravariant)
    beta_tilde = dot_product(metric(1, 2:3), b_contravariant)
    bmag = sqrt(dot_product(b_contravariant, [current_j, current_i]))
