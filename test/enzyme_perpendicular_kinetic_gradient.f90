program enzyme_perpendicular_kinetic_gradient
    use, intrinsic :: iso_c_binding, only: c_double, c_funloc, c_funptr
    use, intrinsic :: iso_fortran_env, only: error_unit
    use perpendicular_kinetic_kernel, only: &
        benchmark_perpendicular_kinetic_energy
    implicit none
    ...
    gradient = enzyme_autodiff( &
        c_funloc(benchmark_perpendicular_kinetic_energy), &
        0.1_c_double)
    centered = (benchmark_perpendicular_kinetic_energy(0.1_c_double + step) &
        - benchmark_perpendicular_kinetic_energy(0.1_c_double - step)) &
        / (2.0_c_double * step)
    if (abs(gradient - centered) > 1.0e-8_c_double &
        * max(1.0_c_double, abs(centered))) then
        write (error_unit, "(a,2es24.16)") &
            "FAIL perpendicular kinetic gradient ", gradient, centered
        error stop 1
    end if
    write (*, "(a)") "PASS"
end program enzyme_perpendicular_kinetic_gradient
