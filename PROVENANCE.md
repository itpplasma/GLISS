...
| `src/terpsichore_matrix_fixture.f90` | Fixed-boundary compatibility readers for the sequential-unformatted `fort.23` records written by the pinned GNU build. The potential-fixture path retains flux/current profiles, radial derivatives, metric/current fields and raw legacy `MODELK`; model decoding remains in the separate policy module, and ingestion remains outside differentiated kernels. The caller must supply and the reader requires `IVAC=0`, because the file header omits `IVAC` although later record shapes depend on it. Default reals are 8 bytes under TERPSICHORE's `-fdefault-real-8` build flag; the source-signed `BJAC` is zero on the unused axis surface and negative on every active cell surface. | TERPSICHORE revision `04dcf9af`, `PARVEC`; shipped GNU Makefile |
