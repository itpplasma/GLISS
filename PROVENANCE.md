...
| `src/physical_mass_kernel.f90` | Pointwise three-component physical kinetic form in `(xi^s, eta, mu)`, including SI mass density. The signed GLISS Jacobian makes `mu` and `sigma_tilde` the negatives of the positive-Jacobian Appendix symbols. | Nuehrenberg, Phys. Plasmas 6 (1999), eqs. A4-A8 |
| `test/enzyme_drive_gradient.f90`, `test/enzyme_phase_assembly_gradient.f90`, `test/enzyme_physical_mass_gradient.f90`, `cmake/EnzymeFortran.cmake` | Enzyme reverse-mode gradients of the local energy, phase assembly, and physical mass kernels | Moses and Churavy, Enzyme (NeurIPS 2020) |
